...
describe('Config Integration Tests', () => {
  ...
  it('should load default configuration when files are missing', async () => {
    // Remove config files
    try {
      await fs.unlink(testConfigPath);
      await fs.unlink(testEnvPath);
    ...
    expect(config.coeConfig.system).toBe('You are a respected member of the Council of Elders. Provide clear, expert guidance.');
  ...
  it('should load custom coe.config.json', async () => {
    const customConfig = {
  ...
  it('should support per-model system prompts', async () => {
