...
import { loadConfig, getModelId, CoeConfig, ModelConfig } from './config.js';
import {
  CouncilClient,
  QueryOptions,
  OpenRouterMessage,
...
program
  ...
  .action(async (options: CliOptions) => {
    try {
      ...
      const _client = new CouncilClient({
      ...
      try {
        availableModels = await _client.getAvailableModels();
        if (availableModels.length > 0) {
          spinner.succeed(`Fetched ${availableModels.length} available models from OpenRouter`);
        } else {
          spinner.warn('No models returned from OpenRouter API');
        }
      } catch (error) {
      ...
      // Config is already validated by Zod in loadConfig
      console.log(chalk.green('  ✓ Schema is valid (validated by Zod)'));
      ...
      if (config.coeConfig.councils) {
        for (const [_councilName, council] of Object.entries(config.coeConfig.councils)) {
          if (council.models) {
      ...
      if (hasIssues) {
        ...
        if (options.interactive && invalidModels.size > 0) {
          ...
          const result = (await prompt.prompt({
            type: 'confirm',
            name: 'shouldFix',
            message: 'Would you like to interactively fix invalid models?',
            initial: true,
          })) as { shouldFix: boolean };

          const shouldFix = result.shouldFix;

          if (shouldFix) {
            // TODO: Implement interactive model replacement
...
program
  ...
  .action(async (promptParts: string[], options: CliOptions) => {
    ...
    let config;
    try {
      config = await loadConfig(options.council);
    } catch (error) {
    ...
    if (
      ...
      !config.coeConfig.models ||
      config.coeConfig.models.length === 0
    ) {
      console.error(chalk.red('Error: Missing configuration'));
      console.error(chalk.yellow('Please run "coe init" to configure'));
      process.exit(1);
    ...
    const activeCouncil =
      options.council || config.userDefaults?.defaultCouncil || config.coeConfig.defaultCouncil;
    const councilDefaults =
      (activeCouncil && config.coeConfig.councils?.[activeCouncil]?.defaults) || {};
    ...
    const outputFormat = options.json
      ? 'json'
      : councilDefaults.json
        ? 'json'
        : config.userDefaults?.json
          ? 'json'
    ...
    const showMeta =
      options.meta !== undefined
        ? options.meta
        : councilDefaults.meta !== undefined
          ? councilDefaults.meta
          : config.userDefaults?.meta !== undefined
            ? config.userDefaults.meta
            : config.coeConfig.output?.showMeta || false;

    const showModels =
      options.showModels !== undefined
        ? options.showModels
        : councilDefaults.showModels !== undefined
          ? councilDefaults.showModels
          : config.userDefaults?.showModels !== undefined
            ? config.userDefaults.showModels
            : config.coeConfig.output?.showModels || false;

    const synthesize =
      options.single !== undefined
        ? options.single
        : councilDefaults.single !== undefined
          ? councilDefaults.single
          : config.userDefaults?.single || false;

    const firstN = options.firstN || councilDefaults.firstN;

    const temperature =
      options.temperature || councilDefaults.temperature || config.userDefaults?.temperature || 0.7;

    // Configure web search with defaults hierarchy
    const webEnabled =
      options.web !== undefined
        ? options.web
        : councilDefaults.web !== undefined
          ? councilDefaults.web
          : config.userDefaults?.web !== undefined
            ? config.userDefaults.web
    ...
    let webSearch:
      | boolean
      | { search_context_size?: string; id?: string; max_results?: number }
      | undefined = undefined;
    if (webEnabled) {
      if (webContext) {
        // Native web search
    ...
    try {
      if (rounds === 1) {
        ...
        const responses = await client.queryMultipleModels(modelIds, messages, queryOptions);
        spinner.stop();
        ...
        if (options.model) {
          console.error(
            chalk.red('Error: Multi-round consensus is not supported with --model option')
          );
        ...
        if (!synthesize) {
          ...
          console.log(builder.format(finalResponses, rounds));
        }
      }
      ...
      if (synthesize) {
        ...
        // Use structured synthesis if available
        const synthesizerModel = config.coeConfig.synthesizer || 'openai/gpt-4o-mini';
        const modelId = getModelId(synthesizerModel);

        // Check if model supports structured output
        const supportsStructured = modelId.includes('gpt-4') || modelId.includes('claude');
        ...
        console.log(builder.format(synthesisResponse));
      }
      ...
      if (options.export) {
        try {
          // Calculate metadata
          const finalResponses = allResponses[allResponses.length - 1];
          const _successfulResponses = finalResponses.filter((r) => !r.error && r.content);
...
`;
  ...
  // Use structured synthesis if supported
  if (useStructured) {
    try {
      const structuredResult = await client.generateStructuredSynthesis(modelId, messages, {
        temperature,
        signal: abortSignal,
      });
      ...
      return {
        model: modelId,
        content: structuredResult.synthesis,
        meta: structuredResult.meta,
      };
    } catch (error) {
      // Fall back to regular synthesis if structured fails
      console.warn(chalk.yellow('Structured synthesis failed, using regular synthesis'));
    }
  }

  return client.queryModel(modelId, messages, {
    temperature,
    signal: abortSignal,
  });
}
  ...
  data.responses.forEach((round, roundIndex) => {
    ...
    round.forEach((response) => {
      if (!response.error || !response.error.includes('first-n limit reached')) {
        const modelName = data.options.showModels
          ? response.model
          : `Elder ${round.indexOf(response) + 1}`;
        md += `#### ${modelName}\n\n`;
